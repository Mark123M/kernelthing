# Batched Cholesky, batch=1 n=4096, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 10** of the gpu-mode
`cholesky` leaderboard: `n: 4096; cond: 2; seed: 48096; batch: 1`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(1, 4096, 4096)` shape with the
...
Input `A` is a `1 x 4096 x 4096` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=4096 does not fit in shared memory — this is a blocked/tiled factorisation (panel + trailing update), and with batch=1 there is little batch parallelism to hide latency with. -->
...
mean time of benchmark index 10 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=4096, batch=1 (target)** | **10** | **? µs** |

Whole-board geometric mean: ? µs. Only index 10 is yours to move — the rest run
