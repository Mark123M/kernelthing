# Batched Cholesky, batch=60 n=1024, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 7** of the gpu-mode
`cholesky` leaderboard: `n: 1024; cond: 2; seed: 511024; batch: 60`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(60, 1024, 1024)` shape with the
...
Input `A` is a `60 x 1024 x 1024` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=1024 does not fit in shared memory — this is a blocked/tiled factorisation (panel + trailing update), and with batch=60 there is little batch parallelism to hide latency with. -->
...
<!-- FILL IN: the shared-memory / occupancy budget at n=1024. A 1024x1024 fp32 matrix is 4.0 MB — far past shared memory, so the blocking factor and the trailing-update GEMM shape are the design. -->
...
mean time of benchmark index 7 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=1024, batch=60 (target)** | **7** | **? µs** |

Whole-board geometric mean: ? µs. Only index 7 is yours to move — the rest run
