# Batched Cholesky, batch=4096 n=32, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 0** of the gpu-mode
`cholesky` leaderboard: `n: 32; cond: 2; seed: 41032; batch: 4096`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(4096, 32, 32)` shape with the
...
Input `A` is a `4096 x 32 x 32` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=32 is small enough that the whole matrix fits in shared memory many times over; batch=4096 means the win is in occupancy and launch overhead, not in blocking. -->
...
<!-- FILL IN: the shared-memory / occupancy budget at n=32. A 32x32 fp32 tile is 4.0 KB — many matrices per block are possible. -->
...
mean time of benchmark index 0 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=32, batch=4096 (target)** | **0** | **? µs** |

Whole-board geometric mean: ? µs. Only index 0 is yours to move — the rest run
