# Batched Cholesky, batch=1024 n=64, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 1** of the gpu-mode
`cholesky` leaderboard: `n: 64; cond: 2; seed: 41064; batch: 1024`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(1024, 64, 64)` shape with the
...
Input `A` is a `1024 x 64 x 64` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=64 is small enough that the whole matrix fits in shared memory many times over; batch=1024 means the win is in occupancy and launch overhead, not in blocking. -->
...
<!-- FILL IN: the shared-memory / occupancy budget at n=64. A 64x64 fp32 tile is 16.0 KB — many matrices per block are possible. -->
...
mean time of benchmark index 1 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=64, batch=1024 (target)** | **1** | **? µs** |

Whole-board geometric mean: ? µs. Only index 1 is yours to move — the rest run
