# Batched Cholesky, batch=640 n=512, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 5** of the gpu-mode
`cholesky` leaderboard: `n: 512; cond: 2; seed: 510512; batch: 640`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(640, 512, 512)` shape with the
...
Input `A` is a `640 x 512 x 512` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=512 does not fit in shared memory — this is a blocked/tiled factorisation (panel + trailing update), and with batch=640 there is little batch parallelism to hide latency with. -->
...
mean time of benchmark index 5 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=512, batch=640 (target)** | **5** | **? µs** |

Whole-board geometric mean: ? µs. Only index 5 is yours to move — the rest run
