# Batched Cholesky, batch=2 n=4096, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 11** of the gpu-mode
`cholesky` leaderboard: `n: 4096; cond: 2; seed: 514096; batch: 2`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(2, 4096, 4096)` shape with the
...
Input `A` is a `2 x 4096 x 4096` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=4096 does not fit in shared memory — this is a blocked/tiled factorisation (panel + trailing update), and with batch=2 there is little batch parallelism to hide latency with. -->
...
<!-- FILL IN: the shared-memory / occupancy budget at n=4096. A 4096x4096 fp32 matrix is 64.0 MB — far past shared memory, so the blocking factor and the trailing-update GEMM shape are the design. -->
...
mean time of benchmark index 11 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=4096, batch=2 (target)** | **11** | **? µs** |

Whole-board geometric mean: ? µs. Only index 11 is yours to move — the rest run
