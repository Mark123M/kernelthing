# Batched Cholesky, batch=4 n=1024, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 6** of the gpu-mode
`cholesky` leaderboard: `n: 1024; cond: 2; seed: 42024; batch: 4`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(4, 1024, 1024)` shape with the
...
Input `A` is a `4 x 1024 x 1024` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=1024 does not fit in shared memory — this is a blocked/tiled factorisation (panel + trailing update), and with batch=4 there is little batch parallelism to hide latency with. -->
...
mean time of benchmark index 6 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=1024, batch=4 (target)** | **6** | **? µs** |

Whole-board geometric mean: ? µs. Only index 6 is yours to move — the rest run
