# Batched Cholesky, batch=1 n=16384, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 13** of the gpu-mode
`cholesky` leaderboard: `n: 16384; cond: 2; seed: 48284; batch: 1`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(1, 16384, 16384)` shape with the
...
Input `A` is a `1 x 16384 x 16384` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=16384 does not fit in shared memory — this is a blocked/tiled factorisation (panel + trailing update), and with batch=1 there is little batch parallelism to hide latency with. -->
...
<!-- FILL IN: the shared-memory / occupancy budget at n=16384. A 16384x16384 fp32 matrix is 1024.0 MB — far past shared memory, so the blocking factor and the trailing-update GEMM shape are the design. -->
...
mean time of benchmark index 13 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=16384, batch=1 (target)** | **13** | **? µs** |

Whole-board geometric mean: ? µs. Only index 13 is yours to move — the rest run
