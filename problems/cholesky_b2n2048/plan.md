# Batched Cholesky, batch=2 n=2048, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 8** of the gpu-mode
`cholesky` leaderboard: `n: 2048; cond: 2; seed: 44048; batch: 2`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(2, 2048, 2048)` shape with the
...
Input `A` is a `2 x 2048 x 2048` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=2048 does not fit in shared memory — this is a blocked/tiled factorisation (panel + trailing update), and with batch=2 there is little batch parallelism to hide latency with. -->
...
mean time of benchmark index 8 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=2048, batch=2 (target)** | **8** | **? µs** |

Whole-board geometric mean: ? µs. Only index 8 is yours to move — the rest run
