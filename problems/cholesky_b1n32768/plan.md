# Batched Cholesky, batch=1 n=32768, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 14** of the gpu-mode
`cholesky` leaderboard: `n: 32768; cond: 2; seed: 48368; batch: 1`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(1, 32768, 32768)` shape with the
...
Input `A` is a `1 x 32768 x 32768` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=32768 does not fit in shared memory — this is a blocked/tiled factorisation (panel + trailing update), and with batch=1 there is little batch parallelism to hide latency with. -->
...
<!-- FILL IN: the shared-memory / occupancy budget at n=32768. A 32768x32768 fp32 matrix is 4096.0 MB — far past shared memory, so the blocking factor and the trailing-update GEMM shape are the design. -->
...
mean time of benchmark index 14 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=32768, batch=1 (target)** | **14** | **? µs** |

Whole-board geometric mean: ? µs. Only index 14 is yours to move — the rest run
