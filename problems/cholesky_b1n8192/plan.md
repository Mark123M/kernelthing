# Batched Cholesky, batch=1 n=8192, on B200

## Goal

Minimise the wall-clock time of benchmark entry **index 12** of the gpu-mode
`cholesky` leaderboard: `n: 8192; cond: 2; seed: 48192; batch: 1`, fp32, on a **B200 (sm_100)**.

`custom_kernel(data)` must handle that exact `(1, 8192, 8192)` shape with the
...
Input `A` is a `1 x 8192 x 8192` CUDA tensor in `torch.float32`; every matrix is
...
<!-- FILL IN: what makes THIS shape different. n=8192 does not fit in shared memory — this is a blocked/tiled factorisation (panel + trailing update), and with batch=1 there is little batch parallelism to hide latency with. -->
...
<!-- FILL IN: the shared-memory / occupancy budget at n=8192. A 8192x8192 fp32 matrix is 256.0 MB — far past shared memory, so the blocking factor and the trailing-update GEMM shape are the design. -->
...
mean time of benchmark index 12 in microseconds. **Lower is better.**
...
| shape | index | time |
|---|---|---|
| **n=8192, batch=1 (target)** | **12** | **? µs** |

Whole-board geometric mean: ? µs. Only index 12 is yours to move — the rest run
